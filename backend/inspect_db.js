  ...
  try {
    ...
    if (companyCount > 0) {
      ...
      companies.forEach((c, i) => {
        ...
        // Count tabs data
        const tabs = c.tabs || {};
        const awardsCount = (tabs.awards && tabs.awards.items && tabs.awards.items.length) || 0;
        const caseStudiesCount = (tabs.caseStudies && tabs.caseStudies.items && tabs.caseStudies.items.length) || 0;
        const productsCount = (tabs.productsServices && tabs.productsServices.items && tabs.productsServices.items.length) || 0;
        const projectsCount = (tabs.projects && tabs.projects.items && tabs.projects.items.length) || 0;
        ...
        console.log(`     Created: ${c.createdAt || (c.timestamps && c.timestamps.createdAt)}`);
      });
    }
    console.log('');
