    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ width: "clamp(180px, 28%, 280px)", minHeight: "220px" }}
        >
          <motion.img
            src={listing.image}
            alt={listing.title}
...
const CommercialRetail = () => {
  ...
  const allListings = listings.concat(
    backendCompanies
      .filter((c) => c.mainSector === 'Comercial / Retail' || c.mainSector === 'Commercial / Retail' || c.mainSector === 'Commercial & Retail' || (Array.isArray(c.industrySector) && c.industrySector.some(s => s.toLowerCase().includes('commercial') || s.toLowerCase().includes('retail'))))
      .map((c) => ({
        id: c.id,
        slug: c.slug,
        title: c.companyName,
    ...
    <div ref={containerRef} className="min-h-screen text-white overflow-hidden" style={{ background: "#040e1e" }}>
      ...
      >
        <div
          className="absolute inset-0"
          style={{
            backgroundImage: "url('/sectors/Commercial.jpeg')",
        ...
        >
          ...
          >
            Powering commercial spaces with sustainable energy solutions that reduce operating costs, enhance brand reputation, and meet ESG goals.
          </motion.p>
          <motion.button
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 md:px-12 pt-20 pb-6">
        ...
        >
          ...
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "clamp(14px, 1.5vw, 16px)", color: "rgba(255,255,255,0.5)", lineHeight: 1.85, fontWeight: 300 }}>
            <span style={{ color: "#10b981", fontWeight: 500 }}>Commercial and retail spaces are embracing renewable energy solutions</span> to cut operating costs, enhance brand reputation, and meet ESG goals. From rooftop solar and heat pumps to smart building management systems, clean technologies are transforming the way offices, shops, and retail parks generate and consume power. Check out some of the UK and Ireland's movers and shakers in the supply of market leading commercial products and services.
          </p>
        </motion.div>
      </div>
