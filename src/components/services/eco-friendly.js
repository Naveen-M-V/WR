...
import { Leaf, ArrowRight, Globe, Check, Image as ImageIcon, ChevronLeft, ChevronRight } from "lucide-react";
...
const SECTOR_LABEL = "Eco Friendly / Passivhaus";
...
export default function EcoFriendly() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        console.error('[EcoFriendly] Error:', err);
  ...
  return (
    <div className="min-h-screen bg-[#040e1e] text-white relative overflow-hidden">
      ...
      {/* Ambient blobs */}
      <div className="absolute top-20 left-20 w-72 h-72 bg-green-500/10 rounded-full blur-[120px] animate-pulse" />
      <div className="absolute bottom-20 right-20 w-96 h-96 bg-emerald-500/10 rounded-full blur-[140px] animate-pulse" />

      <ScrollingBanner />

      {/* HERO */}
      <motion.div ref={heroRef} className="relative w-full overflow-hidden" style={{ height: "clamp(480px,80vh,750px)", y, opacity }}>
        <div className="absolute inset-0" style={{ backgroundImage: "url('/product/Ecofriendly.jpeg')", backgroundSize: "cover", backgroundPosition: "center", filter: "brightness(1.08) saturate(1.2)" }} />
        ...
        <motion.div className="absolute inset-0 flex flex-col justify-end pb-16 px-8 md:px-16 lg:px-24 max-w-6xl"
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.25, duration: 0.6 }}
            ...
            <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "11px", letterSpacing: "0.22em", color: ACCENT, textTransform: "uppercase", fontWeight: 500 }}>
              Sustainable Living
            </span>
          </motion.div>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.7 }}
            ...
            Companies leading the way in eco-friendly, innovative, and environmentally responsible solutions for modern building and domestic housing.
          </motion.p>
          <motion.button initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.6 }}
            ...
            style={{ background: ACCENT, color: "white", boxShadow: "0 0 24px rgba(34,197,94,0.35)" }}>
      ...
      <div id="content" className="relative mt-10 max-w-7xl mx-auto z-10 px-6 pb-20">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-white" style={{ fontFamily: "'Playfair Display',serif" }}>Eco Building Products & Services</h2>
        ...
        {loading ? (
          <div className="text-center py-24 space-y-4">
            ...
            <p className="text-white/30 uppercase tracking-widest text-[10px] font-bold">Synchronizing Eco Hub...</p>
          </div>
        ) : error ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((item, index) => (
              <motion.div key={item.id || index}
                initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }}
                className="glass-panel rounded-[24px] overflow-hidden flex flex-col group transition-all duration-500 hover:border-green-500/30">
                <div className="h-48 relative overflow-hidden">
                  <ItemImageCarousel images={item.images} title={item.title} />
                  <div className="absolute top-3 left-3">
                    <span className={`px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider ${item.type === 'product' ? 'bg-green-500/80 text-white' : 'bg-emerald-500/80 text-white'}`}>
                      {item.type || 'service'}
                    </span>
                  </div>
                </div>
                  ...
                  <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-white/5 border border-white/10 flex items-center justify-center overflow-hidden">
                        {item.companyLogo
                          ? <img src={`${API_HOST}${item.companyLogo}`} className="w-full h-full object-contain" alt="" />
                    ...
                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all glass-panel group-hover:bg-green-500 group-hover:text-white group-hover:border-green-500">
      ...
      <div className="h-1 w-full bg-gradient-to-r from-transparent via-green-500/20 to-transparent" />
    </div>
  );
}
