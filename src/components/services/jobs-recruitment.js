...
import { ArrowRight, Globe, Check, Image as ImageIcon, ChevronLeft, ChevronRight, Briefcase, Building2, Users } from "lucide-react";
...
const SECTOR_LABEL = "Jobs & Recruitment";
...
export default function JobsRecruitment() {
  ...
  const subsectors = [
    ...
    { id: "Internal Company Vacancies", label: "Company Vacancies", icon: Building2 },
    { id: "Recruitment Agencies - Job Vacancies", label: "Recruitment Agencies", icon: Users },
  ];
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        console.error("Error fetching jobs items:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const filtered = activeSubsector === "All" ? items : items.filter(item => item.subsector === activeSubsector);
  ...
  return (
    <div className="min-h-screen bg-[#040e1e] text-white relative overflow-hidden">
      ...
      <div className="absolute top-20 right-20 w-96 h-96 bg-orange-500/10 rounded-full blur-[140px] animate-pulse" />
      <div className="absolute bottom-20 left-20 w-72 h-72 bg-amber-500/10 rounded-full blur-[120px] animate-pulse" />

      <ScrollingBanner />

      {/* HERO */}
      <motion.div ref={heroRef} className="relative w-full overflow-hidden mb-20" style={{ height:"clamp(480px,80vh,750px)" }}>
        <div className="absolute inset-0" style={{ backgroundImage:"url('/product/jbr.jpeg')", backgroundSize:"cover", backgroundPosition:"center", filter:"brightness(1.08) saturate(1.2)" }} />
        ...
        <motion.div className="absolute inset-0 flex flex-col justify-end pb-16 px-8 md:px-16 lg:px-24 max-w-6xl" initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ duration:0.8 }}>
          <motion.div initial={{ opacity:0, x:-20 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.25, duration:0.6 }} className="flex items-center gap-3 mb-6">
            ...
            <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:"11px", letterSpacing:"0.22em", color: ACCENT, textTransform:"uppercase", fontWeight:500 }}>Talent Hub</span>
          </motion.div>
          ...
          <motion.p initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.6, duration:0.7 }}
            ...
            Job Vacancies and recruitment services across the renewable and sustainability sectors.
          </motion.p>
          <motion.button initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.8, duration:0.6 }}
            ...
            style={{ background: ACCENT, color:"white", boxShadow:"0 0 24px rgba(249,115,22,0.35)" }}>
      ...
      <div id="content" className="max-w-7xl mx-auto px-6 pb-32">
          ...
          {subsectors.map(tab => {
            ...
            return (
              <button key={tab.id} onClick={() => setActiveSubsector(tab.id)}
                className={`flex items-center gap-3 px-6 py-4 rounded-2xl transition-all duration-300 glass-panel ${isActive ? 'border-orange-500/50 bg-orange-500/10' : 'hover:bg-white/5'}`}>
                <Icon size={18} className={isActive ? "text-orange-400" : "text-white/30"} />
        ...
        <div className="text-right mb-6">
          <span className="text-xs text-white/30">{filtered.length} listing{filtered.length !== 1 ? 's' : ''}</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-24 text-center glass-panel rounded-[32px]">
            ...
            <p className="text-white/30 uppercase tracking-widest text-xs font-bold">No listings found</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((item, index) => (
              <motion.div key={item.id || index} initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ delay: index * 0.05 }}
                className="glass-panel rounded-[24px] overflow-hidden flex flex-col group transition-all duration-500 hover:border-orange-500/30">
                <div className="h-48 relative overflow-hidden">
                  <ItemImageCarousel images={item.images} title={item.title} />
                  <div className="absolute top-3 left-3">
                    <span className="px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider bg-orange-500/80 text-white">
                      {item.type || 'vacancy'}
                    </span>
                  </div>
                </div>
                <div className="flex-1 p-6 flex flex-col">
                  {item.subsector && <p className="text-[10px] font-bold uppercase tracking-widest mb-2 text-orange-400">{item.subsector}</p>}
                  <h3 className="text-lg font-bold text-white mb-2 group-hover:text-orange-400 transition-colors" style={{ fontFamily:"'Playfair Display',serif" }}>{item.title}</h3>
                  ...
                  <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-white/5 border border-white/10 flex items-center justify-center overflow-hidden">
                        {item.companyLogo ? <img src={`${API_HOST}${item.companyLogo}`} className="w-full h-full object-contain" alt="" /> : <Globe size={12} className="text-orange-400" />}
                      </div>
                    ...
                    <motion.button whileHover={{ scale:1.05 }} whileTap={{ scale:0.95 }} onClick={() => {
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all glass-panel group-hover:bg-orange-500 group-hover:text-white group-hover:border-orange-500">
