...
import { ArrowRight, Leaf, ShieldCheck, BarChart3, Globe, Check, Image as ImageIcon, ChevronLeft, ChevronRight } from "lucide-react";
...
const SECTOR_LABEL = "Sustainable / ESG / Net Zero";
...
export default function ContentPage() {
  ...
  const subsectors = [
    ...
    { id: "Hardware Products & Solutions", label: "Eco Hardware", icon: Leaf },
    { id: "Sustainable & ESG Services", label: "ESG Strategy", icon: ShieldCheck },
    { id: "Net Zero/Carbon Industry", label: "Net Zero Path", icon: BarChart3 },
  ];
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        console.error("Error fetching sustainable items:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const filtered = activeTab === "All" ? items : items.filter(item => item.subsector === activeTab);

  return (
    <div className="min-h-screen" style={{ background: "#040e1e", color: "#ffffff" }}>
      <style>{`
        ...
        .premium-blur { backdrop-filter:blur(16px) saturate(180%); background-color:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); }
      ...
      <motion.div ref={heroRef} className="relative w-full overflow-hidden mb-20" style={{ height:"clamp(480px,80vh,750px)" }}>
        <div className="absolute inset-0" style={{ backgroundImage:"url('/product/Sustainibility.jpeg')", backgroundSize:"cover", backgroundPosition:"center", filter:"brightness(1.08) saturate(1.2)" }} />
        ...
        <motion.div className="absolute inset-0 flex flex-col justify-end pb-16 px-8 md:px-16 lg:px-24 max-w-6xl" initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ duration:0.8 }}>
          <motion.div initial={{ opacity:0, x:-20 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.25, duration:0.6 }} className="flex items-center gap-3 mb-6">
            ...
            <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:"11px", letterSpacing:"0.22em", color: ACCENT, textTransform:"uppercase", fontWeight:500 }}>Future Compliance</span>
          </motion.div>
          <motion.h1 initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.4, duration:0.8 }} className="hero-text-mask mb-5"
            style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(2.8rem,4vw,5.5rem)", fontWeight:700, lineHeight:1.35, letterSpacing:"-0.02em" }}>
            Sustainability & ESG
          </motion.h1>
          <motion.p initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.6, duration:0.7 }}
            ...
            Connecting enterprise leaders with specialised services to meet global environmental mandates.
          </motion.p>
          <motion.button initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.8, duration:0.6 }}
            ...
            style={{ background: ACCENT, color:"white", boxShadow:"0 0 24px rgba(16,185,129,0.35)" }}>
      ...
      <div id="content" className="max-w-7xl mx-auto px-6 pb-32">
          ...
          {subsectors.map(tab => {
            ...
            return (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`flex items-center gap-3 px-6 py-4 rounded-2xl transition-all duration-500 premium-blur ${isActive ? 'border-emerald-500/50 bg-emerald-500/10' : 'hover:bg-white/5'}`}>
                <Icon size={18} className={isActive ? "text-emerald-400" : "text-white/30"} />
        ...
        <div className="text-right mb-6">
          <span className="text-xs text-white/30" style={{ fontFamily:"'DM Sans',sans-serif" }}>{filtered.length} item{filtered.length !== 1 ? 's' : ''}</span>
        </div>

        {/* ITEMS GRID */}
        {loading ? (
          <div className="py-24 text-center text-white/20 uppercase tracking-[0.3em] text-xs">Accessing Directory...</div>
        ) : filtered.length === 0 ? (
          <div className="py-24 text-center premium-blur rounded-[32px]">
          ...
          <AnimatePresence mode="popLayout">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((item, index) => (
                <motion.div key={item.id || index} layout initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ delay: index * 0.05 }}
                  className="premium-blur rounded-[24px] overflow-hidden flex flex-col hover:border-emerald-500/30 transition-all duration-500 group">
                  <div className="h-48 relative overflow-hidden">
                    <ItemImageCarousel images={item.images} title={item.title} />
                    <div className="absolute top-3 left-3">
                      <span className={`px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider ${item.type === 'product' ? 'bg-emerald-500/80 text-white' : 'bg-cyan-500/80 text-white'}`}>
                        {item.type || 'service'}
                      </span>
                    </div>
                  </div>
                  <div className="flex-1 p-6 flex flex-col">
                    {item.subsector && <p className="text-[10px] font-bold uppercase tracking-widest mb-2 text-emerald-400">{item.subsector}</p>}
                    <h3 className="text-lg font-bold text-white mb-2 group-hover:text-emerald-400 transition-colors" style={{ fontFamily:"'Playfair Display',serif" }}>{item.title}</h3>
                    ...
                    <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full bg-white/5 border border-white/10 flex items-center justify-center overflow-hidden">
                          {item.companyLogo ? <img src={item.companyLogo} className="w-full h-full object-contain" alt="" /> : <Globe size={12} className="text-white/30" />}
                        </div>
                      ...
                      <motion.button whileHover={{ scale:1.05 }} whileTap={{ scale:0.95 }} onClick={() => {
                        ...
                        className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all glass-panel group-hover:bg-amber-500 group-hover:text-white group-hover:border-amber-500">
