      ...
      >
        ...
        <div
          ...
          style={{
            ...
            boxShadow: hovered ? `0 0 14px rgba(${accent.rgb},0.4)` : "none",
          }}
        >
...
export default function ProductsSpotlight() {
  ...
  const goToCompanyProductsServices = (companyId) => {
    if (!companyId) return;
    navigate(`/company/${encodeURIComponent(companyId)}?tab=products`);
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {products.slice(0, 6).map((product, index) => (
              <ProductCard
                ...
                product={product}
                index={index}
                onClick={() => goToCompanyProductsServices(product.companyId)}
              />
            ))}
          </div>
        ) : (
